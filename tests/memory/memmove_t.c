...
static void cb(void *pr) {
	...
	memmove_c(p->buf, p->buflen, p->from, p->fromlen);
}
...
void check(u8 *buf, ulen buflen, const u8 *sample, ulen samplelen) {
	...
	assert(memmove(buf + 1, sample, samplelen) == buf + 1);
	...
	for (ulen tbuflen = 0; tbuflen <= buflen - 2; tbuflen++) {
		...
		if (tbuflen < samplelen) {
			...
			assert(memmove_c(buf + 1, tbuflen, sample, samplelen) == buf + 1);
			assert(memeq(buf + 1, sample, samplelen));
		}
...
void checkranges(u8 *buf) {
	...
	for (u8 **start = interesting_cases; *start != NULL; start++) {
		for (u8 **end = start + 1; *end != NULL; end++) {
			...
			if (reallen <= usedbufferlen) {
				assert(memmove_c(usedbuffer, usedbufferlen, realsource, reallen) == usedbuffer);
			}
		}
	}
}
...
CAMAIN0() {
	...
	// checking for memmove_c checking for nullity
	...
	// checking for memmove_c checking the ranges
	checkranges(buf);

	// checking that overlapped moves complete properly
	for (ulen off = 5; off <= 15; off++) {
		assert(memset(buf, 0, 64) == buf);
	
		assert(memcpy(buf + 10, sourcedata, 32) == buf + 10);
		assert(memmove(buf + off, buf + 10, 32) == (buf + off));
		assert(memeq(buf + off, sourcedata, 32));

		for (ulen i = 10; i < 10 + 32; i++) {
			if (i < off || i >= off + 32) {
				assert(buf[i] == sourcedata[i - 10]);
			}
		}
	
		for (ulen i = 0; i < off && i < 10; i++) {
			assert(buf[i] == 0);
		}
		ulen zerozone = 10 + 32;
		if (off + 32 > zerozone) {
		...
		for (ulen i = zerozone; i < zerozone + 5; i++) {
			assert(buf[i] == 0);
		}
	}
